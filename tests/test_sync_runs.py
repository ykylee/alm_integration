...
def test_retry_cancelled(store: SyncRunStore) -> None:
    original = store.create("jira", None, None)
    original.run_status = "cancelled"

    retried, err = store.retry(original.run_id, reason="retry")
...
def test_retry_partially_completed(store: SyncRunStore) -> None:
    original = store.create("jira", None, None)
    original.run_status = "partially_completed"
