...
2026-04-08 후속 정리 기준으로 조직 관리와 인력 관리 페이지는 단순히 “데이터가 보이는 화면”이 아니라, 사용자가 페이지에서 답을 얻고 싶은 질문 중심으로 구성한다.

- 조직 관리에서 사용자가 먼저 알고 싶은 것:
  - 이 조직이 어디에 속하는가
  - 이 조직 아래에 무엇이 얼마나 있는가
  - 이 조직에 누가 직접 속해 있는가
  - 이 조직을 바꾸면 무엇이 막히거나 영향을 받는가
- 조직 관리에서 사용자가 바로 하고 싶은 것:
  - 조직 등록/수정/삭제
  - 상위 조직 변경
  - 직속 구성원 등록/이동/비활성화
  - 최근 조직 변경/구성원 이동 이력 확인
- 인력 관리에서 사용자가 먼저 알고 싶은 것:
  - 지금 보고 있는 인력이 어느 조직 소속인가
  - 이 인력이 어떤 참조 축으로 쓰이는가
  - 같은 조직 안에서 어떤 역할 분포를 갖는가
  - 이동 또는 비활성화 시 어떤 후속 확인이 필요한가
- 인력 관리에서 사용자가 바로 하고 싶은 것:
  - 인력 검색과 조직 범위 좁히기
  - 선택 인력 상세 확인
  - 조직 이동 또는 비활성화
  - 신규 등록과 기본 인적 정보 정정

이 기준에 따라 조직 관리 화면은 `이 조직을 이해하기 -> 이 조직의 직속 구성원 -> 변경 전에 확인할 영향 -> 구조 변경/구성원 배치 변경 액션` 흐름으로, 인력 관리 화면은 `선택 인력 인스펙터 -> 인력 목록 -> 조직 맥락과 운영 판단 -> 구성원 액션` 흐름으로 읽히게 보강한다.
...
2026-04-08 기준 `organization.html`, `admin.html` 과 데이터 관리 하위 페이지들은 정적 설명 화면에 머물지 않고 실제 운영 API 와 연결할 수 있는 최소 바인딩을 갖는다. 기본 API 기준 URL 은 `http://127.0.0.1:8080/api/v1` 이며, 현재는 운영 화면이 아니라 [../../src/ui_prototype/data_settings.html](../../src/ui_prototype/data_settings.html) 에서 연결 설정과 연결 확인을 일괄 관리한다.
