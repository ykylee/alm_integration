...
- 현재 Rust 런타임은 `POST /api/v1/ingestion/events` 에 대해 `X-Source-System`, `X-Signature`, `X-Signature-Timestamp`, `X-Idempotency-Key` 를 검사한다.
- 서명 문자열은 `timestamp + "\\n" + method + "\\n" + path + "\\n" + sha256(body)` 형식을 사용한다.
- `HMAC-SHA256` 결과는 `sha256=<hex>` 형식 헤더로 비교한다.
- 허용 시간 편차는 기본 300초다.
- source 별 서명 secret 이 등록되지 않은 환경에서는 기존 호환을 위해 무서명 수신을 허용한다.
...
- 현재 구현은 `enc-v1:<base64>` 형식의 암호문을 애플리케이션에서 복호화한다.
- 키 조회는 우선 `ALM_BACKEND_SECRET_KEY_<SECRET_KEY_VERSION>` 환경변수를 사용하고, 버전이 없으면 `ALM_BACKEND_SECRET_KEY` 를 사용한다.
- 레거시 평문 또는 `plain:` 접두 형식은 이행 기간 호환을 위해 읽기는 허용하지만, 신규 운영 데이터는 암호문 저장을 기준으로 본다.
